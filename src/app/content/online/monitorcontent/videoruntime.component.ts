...
@Component({
  ...
  templateUrl:'./videoruntime.component.html'
})
export class VideoRunTimeComponent implements OnInit{
  ...
  getList(){
    ...
    if(this.projectId && this.crane){
      ...
      this.online.getCraneVideoRunTime(this.projectId,this.crane['craneId'],params).subscribe((res)=>{
        if(res){
          if(res[0]){
            this.videoRunTime = [];
